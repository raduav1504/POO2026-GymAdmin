...
GroupClass::GroupClass(const std::string& name_, int capacitate,const std::string& instructor_, int durata)
    : GymService(name_, durata), capacitateMax{capacitate}, instructor{instructor_} {}
...
std::string GroupClass::calculeazaRezultat() const {
    ...
    return "Ocupare: " + std::to_string(participanti) + "/" + std::to_string(capacitateMax) + " (" + std::to_string(rata) + "%)";
}
...
void GroupClass::afisare(std::ostream& os) const {
    ...
    os << " | Instructor: " << instructor << " | Locuri libere: " << getLocuriLibere() << "/" << capacitateMax;
}
